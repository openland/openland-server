...
export default {
    ...
    Mutation: {
        registerWebPush: async (_: any, args: { endpoint: string }, ctx: AppContext) => {
            ...
            await Modules.Push.registerPushWeb(ctx.auth.uid!, ctx.auth.tid!, args.endpoint);
            return 'ok';
        },
        registerPush: async (_: any, args: { endpoint: string, type: 'WEB_PUSH' | 'IOS' | 'ANDROID' }, ctx: AppContext) => {
            ...
            return await withLogContext('push', async () => {
                ...
                if (args.type === 'IOS') {
                    ...
                    await Modules.Push.registerPushApple(ctx.auth.uid!, ctx.auth.tid!, parsed.token, parsed.bundleId, parsed.sandbox);
                    return 'ok';
                }
                if (args.type === 'ANDROID') {
                    let parsed = JSON.parse(args.endpoint);
                    await Modules.Push.registerPushAndroid(ctx.auth.uid!, ctx.auth.tid!, parsed.token, parsed.bundleId, parsed.sandbox);
                    return 'ok';
                }
