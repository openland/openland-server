...
export class PresenceUserService {
    ...
    async start() {
        ...
        this.subscription = EventBus.subscribe('presence', `presences.users-notify.${this.user.uid}`, () => {
            this.sync.invalidate();
        });
    }
    ...
    doSync = async () => {
        ...
        EventBus.publish('presence', `presences.user.${this.user.uid}`, online.online);
        ...
        for (let g of groups) {
            EventBus.publish('presence', `presences.groups-notify.${g}`, { uid: this.user.uid, timeout: online.online.lastSeen.timeout, readVersiion: online.readVersiion.toString('hex') });
        }
    }
