...
export class IdEntity extends FEntity {
    ...
    get ref(): FID | null {
        let res = this._value.ref;
        if (res) { return unpackFID(res); }
        return null;
    }
    set ref(value: FID | null) {
        this._checkIsWritable();
        if (value) {
            let converted = packFID(value);
            if (converted === this._value.ref) { return; }
            this._value.ref = value;
            this.markDirty();
        } else {
            if (!this._value.ref) { return; }
...
export class IdEntityFactory extends FEntityFactory<IdEntity> {
    private static validate(src: any) {
        validators.notNull('id', src.id);
        validators.isId('id', src.id);
        validators.notNull('data', src.data);
        validators.isString('data', src.data);
        validators.isId('ref', src.ref);
    }

    constructor(connection: FConnection) {
        super(connection,
            new FNamespace('entity', 'idEntity'),
            { enableVersioning: false, enableTimestamps: false, validator: IdEntityFactory.validate, hasLiveStreams: false },
            [],
            'IdEntity'
        );
    }
    ...
    async create(id: FID, shape: IdEntityShape) {
    ...
    protected _createEntity(value: any, isNew: boolean) {
        return new IdEntity(this.connection, this.namespace, [value.id], value, this.options, isNew, this.indexes, 'IdEntity');
    }
}
...
export class AllEntities extends FDBInstance {
    ...
    constructor(connection: FConnection) {
        ...
        this.IdEntity = new IdEntityFactory(connection);
    }
}
