...
describe('Partial Index', () => {
    ...
    it('should create indexes if condition true', async () => {
        ...
        await withLogDisabled(async () => {
            let res1 = await inTx(async () => { return await testEntities.IndexedPartialEntity.create(ctx, 0, { data1: 'hello', data2: 'world', data3: '' }); });
            expect(res1.data1).toEqual('hello');
            expect(res1.data2).toEqual('world');
            let res2 = (await testEntities.IndexedPartialEntity.findFromDefault(ctx, 'hello', 'world', 0))!;
            ...
            let res3 = (await inTx(async () => { return testEntities.IndexedPartialEntity.findFromDefault(ctx, 'hello', 'world', 0); }))!;
    ...
    it('should not create indexes if condition false', async () => {
        let ctx = createEmptyContext();
        await withLogDisabled(async () => {
            let res1 = await inTx(async () => { return await testEntities.IndexedPartialEntity.create(ctx, 1, { data1: 'hello2', data2: 'world', data3: '' }); });
            expect(res1.data1).toEqual('hello2');
            expect(res1.data2).toEqual('world');
            let res2 = (await testEntities.IndexedPartialEntity.findFromDefault(ctx, 'hello2', 'world', 1))!;
            expect(res2).toBeNull();

            let res3 = (await inTx(async () => { return testEntities.IndexedPartialEntity.findFromDefault(ctx, 'hello2', 'world', 0); }))!;
    ...
    it('should create indexes if condition was changed from false to true', async () => {
        let ctx = createEmptyContext();
        await withLogDisabled(async () => {
            let res1 = await inTx(async () => { return await testEntities.IndexedPartialEntity.create(ctx, 3, { data1: 'hello2', data2: 'world', data3: '' }); });
            expect(res1.data1).toEqual('hello2');
            expect(res1.data2).toEqual('world');
            ...
            expect(await testEntities.IndexedPartialEntity.findFromDefault(ctx, 'hello2', 'world', 3)).toBeNull();

            let res4 = (await testEntities.IndexedPartialEntity.findFromDefault(ctx, 'hello', 'world', 3))!;
            ...
            let res5 = (await inTx(async () => { return testEntities.IndexedPartialEntity.findFromDefault(ctx, 'hello', 'world', 3); }))!;
    ...
    it('should delete indexes if condition was changed from true to false', async () => {
        let ctx = createEmptyContext();
        await withLogDisabled(async () => {
            let res1 = await inTx(async () => { return await testEntities.IndexedPartialEntity.create(ctx, 4, { data1: 'hello', data2: 'world', data3: '' }); });
            expect(res1.data1).toEqual('hello');
            expect(res1.data2).toEqual('world');
            ...
            expect(await testEntities.IndexedPartialEntity.findFromDefault(ctx, 'hello', 'world', 4)).toBeNull();
            expect(await testEntities.IndexedPartialEntity.findFromDefault(ctx, 'hello2', 'world', 4)).toBeNull();
        });
    });
});