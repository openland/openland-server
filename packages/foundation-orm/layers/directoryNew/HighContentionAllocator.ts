...
export class HighContentionAllocator {
    ...
    constructor(subspace: Buffer) {
        this.counters = Buffer.concat([subspace, FEncoders.tuple.pack([0])]);
        this.recent = Buffer.concat([subspace, FEncoders.tuple.pack([1])]);
    }

    async allocate(ctx: Context, connection: FConnection) {
        let tx = getTransaction(ctx).rawTransaction(connection);
        while (true) {
            let rr = await tx.snapshot().getRangeAllStartsWith(this.counters, { limit: 1, reverse: true });
            let start = 0;
            let window = 0;
            if (rr.length === 1) {
                let ex = FEncoders.tuple.unpack(rr[0][0].slice(this.counters.length));
            ...
            while (true) {
                ...
                if (windowAdvanced) {
                    tx.clearRange(this.counters, Buffer.concat([this.counters, FEncoders.tuple.pack([start])]));
                    tx.setOption(TransactionOptionCode.NextWriteNoWriteConflictRange);
                    tx.clearRange(this.recent, Buffer.concat([this.recent, FEncoders.tuple.pack([start])]));
                }
                tx.add(Buffer.concat([this.counters, FEncoders.tuple.pack([start])]), increment);
                let newCountPromise = tx.snapshot().get(Buffer.concat([this.counters, FEncoders.tuple.pack([start])]));
                // }
            ...
            while (true) {
                ...
                let key = Buffer.concat([this.recent, FEncoders.tuple.pack([candidate])]);
