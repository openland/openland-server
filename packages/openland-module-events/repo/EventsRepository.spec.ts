...
describe('EventsRepository', () => {
    ...
    it('should post events and get changed feeds', async () => {
        ...
        changed = await repo.getChangedFeeds(root, subs1, await second1.state);
        expect(changed.events.length).toBe(0);
        expect(changed.changes.length).toBe(0);
        changed = await repo.getChangedFeeds(root, subs2, await second2.state);
        expect(changed.events.length).toBe(0);
        expect(changed.changes.length).toBe(0);
        ...
        await repo.post(root, { feed: feed1, event: ZERO });
        changed = await repo.getChangedFeeds(root, subs1, await second1.state);
        expect(changed.events.length).toBe(1);
        expect(changed.changes.length).toBe(0);
        changed = await repo.getChangedFeeds(root, subs2, await second2.state);
        expect(changed.events.length).toBe(1);
        expect(changed.changes.length).toBe(0);
    ...
    it('should post events and get changed feeds async', async () => {
        let root = createNamedContext('test');
        let db = await Database.openTest({ name: 'event-events-changed-async', layers: [] });
        let repo = new EventsRepository(db.allKeys);
        let feed1 = await repo.createFeed(root);
        let feed2 = await repo.createFeed(root);
        let subs1 = await repo.createSubscriber(root);
        ...
        // Subscribe
        await repo.subscribe(root, subs1, feed1, { mode: 'async', strict: false });
