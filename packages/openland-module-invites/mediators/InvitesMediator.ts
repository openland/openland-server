    ...
    async joinAppInvite(parent: Context, uid: number, inviteStr: string, isNewUser: boolean) {
        return await inTx(parent, async (ctx) => {
            ...
            await Modules.Messaging.sendMessage(
                ...
                chat.id,
                supportUserId,
                buildServiceMessage(userMention(name1, uid), ' accepted your invite. Now you can\u00A0chat!'),
                true
            );
            return 'ok';
        });
        ...
        return await inTx(parent, async (ctx) => {
            ...
            if (supportUserId && !isActivatedAlready) {
                await Modules.Messaging.sendMessage(
                    ...
                    chat.id,
                    supportUserId,
                    buildServiceMessage(`🙌 `, userMention(name2, invite.uid), ' — ', userMention(name1, uid), ' has accepted your invite. Now you can chat!'),
                    true
                );
            }
