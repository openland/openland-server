...
describe('MessagingMediator', () => {
    ...
    it('should send message', async () => {
        ...
        let room = await roooms.createRoom(rootCtx, 'public', org.id, USER_ID, [], { title: 'Room' });

        let text = 'boom';
        let message = await inReadOnlyTx(rootCtx, async (ctx) => (await Store.Message.findById(ctx, (await mediator.sendMessage(rootCtx, USER_ID, room.id, { message: text })).id))!);
    ...
    it('should set and reset reaction', async () => {
        ...
        let text = 'boom';
        let MSG_ID = (await mediator.sendMessage(rootCtx, USER_ID, room.id, { message: text })).id;

        await mediator.setReaction(rootCtx, MSG_ID, USER_ID, 'LIKE');
        let message = await inReadOnlyTx(rootCtx, async (ctx) => (await Store.Message.findById(ctx, MSG_ID))!);
        ...
        await mediator.setReaction(rootCtx, MSG_ID, USER_ID, 'LIKE', true);
        message = await inReadOnlyTx(rootCtx, async (ctx) => (await Store.Message.findById(ctx, MSG_ID))!);
    ...
    it('should edit message', async () => {
        ...
        let MSG_ID = (await mediator.sendMessage(rootCtx, USER_ID, room.id, { message: 'boom' })).id;

        let message = await inReadOnlyTx(rootCtx, async (ctx) => (await Store.Message.findById(ctx, MSG_ID))!);
        let textResolved = await ChatResolver.Resolver.ConversationMessage!.message!(message, {}, {} as any);
        expect(textResolved).toEqual('boom');

        await mediator.editMessage(rootCtx, MSG_ID, USER_ID, { message: 'boom shakalaka' }, false);
    ...
    it('should delete url augmentation', async () => {
        ...
        let MSG_ID = (await mediator.sendMessage(rootCtx, USER_ID, room.id, { message: 'boom', attachments: [richAttachment] })).id;
        ...
        await mediator.editMessage(rootCtx, MSG_ID, USER_ID, { attachments: newAttachments }, false);
    ...
    it('history pagination should work correctly', async () => {
        ...
        for (let i = 0; i < 4; i++) {
            await inReadOnlyTx(rootCtx, async (ctx) => await Store.Message.findById(ctx, (await mediator.sendMessage(rootCtx, USER_ID, room.id, { message: i.toString() })).id));
        }

        // load history first pack
        let range = await inReadOnlyTx(rootCtx, async (ctx) => (await Store.Message.chat.query(ctx, room.id, { limit: 2, reverse: true })).items);
        expect(range[0].text).toEqual('3');
        expect(range[1].text).toEqual('2');

        // load range before oldest message in prev range
        range = await inReadOnlyTx(rootCtx, async (ctx) => (await Store.Message.chat.query(ctx, room.id, { after: range[1].id, limit: 2, reverse: true })).items);
