...
function decrypt(valuestr: string, value: Buffer, type: number | Set<number>, encryptionKey: Buffer, encryptionIv: Buffer, hmacKey: Buffer) {
    ...
    let valueType = decoded.readInt8(3);
    let correctValueTypeId = false;
    let valueRes: SecIDv2ValueType|undefined;

    if (valueType === NUMBER_TYPE) {
        correctValueTypeId = true;
        valueRes = decoded.readUInt32BE(4);
    } else if (valueType === STRING_TYPE) {
        correctValueTypeId = true;
        let stringLen = decoded.readUInt16BE(4);
        valueRes = decoded.slice(6, 6 + stringLen).toString('utf-8');
    }
...
export class SecIDv2<T extends SecIDv2ValueType> {
...
export class SecIDv2Factory {
    ...
    private knownSecIDS = new Map<number, SecIDv2<any>>();
    ...
    createNumberId(type: string) {
        return this.createId<number>(type, 'number');
    }

    createStringId(type: string) {
        return this.createId<string>(type, 'string');
    }
    ...
    private createId<T extends SecIDv2ValueType>(type: string, valueType: SecIDv2ValueTypeName) {
        ...
        let id = new SecIDv2<T>(type, typeId, valueType, this.encryptionKey, this.encryptionIv, this.hmacKey, this.style, this.hashids);
